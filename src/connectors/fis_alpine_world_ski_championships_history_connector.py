...
SEED_FILE = "fis_alpine_world_ski_championships_top3_seed.csv"
COMPETITION_ID = "fis_alpine_world_ski_championships"
SPORT_ID = "skiing"
DISCIPLINE_ID = "alpine-skiing"
...
class FisAlpineWorldSkiChampionshipsHistoryConnector(Connector):
    id = "fis_alpine_world_ski_championships_history"
    name = "FIS Alpine World Ski Championships Historical Podiums"
    source_type = "csv"
    license_notes = (
        "Historical podium seed curated from public Wikipedia FIS Alpine World Ski Championships medalist "
        "tables. Verify downstream redistribution requirements."
    )
    base_url = "https://en.wikipedia.org/wiki/List_of_FIS_Alpine_World_Ski_Championships_medalists"
    ...
    def source_row(self) -> dict[str, str]:
        return {
            ...
            "license_notes": (
                "Local seed data/raw/skiing/fis_alpine_world_ski_championships_top3_seed.csv built from "
                "Wikipedia medalist list tables, with 2003 supplemented from the annual results page because "
                "the list page has empty medal cells for that edition. Strict post-2000 scope; official medal "
                "ties are preserved."
            ),
    ...
    def fetch(self, season_year: int, out_dir: Path) -> list[Path]:
        ...
        if not local_seed.exists():
            raise RuntimeError(f"Missing local seed for FIS Alpine Worlds: {local_seed}")

        frame = pd.read_csv(local_seed)
        if "year" not in frame.columns:
            raise RuntimeError(f"Unsupported FIS Alpine seed format for {local_seed.name}: missing `year`.")
        ...
        if filtered.empty:
            raise RuntimeError(f"No FIS Alpine rows available for year <= {season_year} and year > 2000.")
        ...
        self._write_json(
            out_dir / "fetch_meta.json",
            {
                ...
                "source_note": "2003 rows are supplemented from the annual 2003 page because the medalist list page is blank for that edition.",
            },
        )
        return [out_file]
    ...
    def parse(self, raw_paths: list[Path], season_year: int) -> dict[str, pd.DataFrame]:
        ...
        if not required_cols.issubset(set(frame.columns)):
            raise RuntimeError(f"Unsupported FIS Alpine seed format for {seed_path.name}: {list(frame.columns)}")
        ...
        if bad_profiles:
            ...
            raise RuntimeError(f"Unexpected FIS Alpine rank profiles: {sample}")
        ...
        for row in frame.itertuples(index=False):
            ...
            participant_prefix = "fis_alpine_team" if participant_type == "team" else "fis_alpine_athlete"
            ...
            rank = int(row.rank)
            team_members = str(row.team_members).strip()
            ...
            results_rows.append(
                {
                    ...
                    "medal": str(row.medal),
                    "score_raw": (
                        f"event={row.event_name};entry_country={country_code};"
                        f"team_members={team_members};source_url={row.source_url}"
                    ),
    ...
    def _assert_result_profiles(results_df: pd.DataFrame) -> None:
        ...
        if bad_profiles:
            sample = dict(list(bad_profiles.items())[:30])
            raise RuntimeError(f"Unexpected FIS Alpine result profiles after participant build: {sample}")
