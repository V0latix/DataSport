...
SEED_FILE = "biathlon_world_championships_top3_seed.csv"
...
class BiathlonWorldChampionshipsHistoryConnector(Connector):
    id = "biathlon_world_championships_history"
    name = "Biathlon World Championships Historical Podiums"
    source_type = "csv"
    license_notes = (
        "Historical podium seed curated from public Wikipedia Biathlon World Championships medalist tables. "
        "Verify downstream redistribution requirements."
    )
    base_url = "https://en.wikipedia.org/wiki/Biathlon_World_Championships"
    ...
    def source_row(self) -> dict[str, str]:
        return {
            ...
            "license_notes": (
                "Local seed data/raw/biathlon/biathlon_world_championships_top3_seed.csv built from Wikipedia "
                "Biathlon World Championships event medalist tables. Strict post-2000 scope; relay events are "
                "modeled as nation teams with team members retained in score metadata. Expected rank profile "
                "is 1,2,3 except source-official 2003 women's pursuit where silver is not awarded."
            ),
            "base_url": self.base_url,
        }

    def _local_seed_path(self) -> Path:
        return Path(__file__).resolve().parents[2] / "data" / "raw" / "biathlon" / SEED_FILE
    ...
    def fetch(self, season_year: int, out_dir: Path) -> list[Path]:
        ...
        if not local_seed.exists():
            raise RuntimeError(f"Missing local seed for Biathlon World Championships: {local_seed}")

        frame = pd.read_csv(local_seed)
        if "year" not in frame.columns:
            raise RuntimeError(f"Unsupported Biathlon World Championships seed format for {local_seed.name}.")
        ...
        if filtered.empty:
            raise RuntimeError(f"No Biathlon World Championships rows for year <= {season_year} and year > 2000.")
        ...
        self._write_json(
            out_dir / "fetch_meta.json",
            {
                ...
                "source_note": "2003 women's pursuit has no silver medal awarded in the source table.",
            },
        )
        return [out_file]
    ...
    def parse(self, raw_paths: list[Path], season_year: int) -> dict[str, pd.DataFrame]:
        ...
        if not required_cols.issubset(set(frame.columns)):
            raise RuntimeError(f"Unsupported Biathlon World Championships seed columns: {list(frame.columns)}")
        ...
        if (frame["year"] <= 2000).any():
            ...
            raise RuntimeError(f"Post-2000 guard violated for Biathlon World Championships seed: {offenders}")
        ...
        if bad_profiles:
            ...
            raise RuntimeError(f"Unexpected Biathlon World Championships rank profiles: {sample}")

        timestamp = utc_now_iso()
        sports_df = pd.DataFrame(
            [{"sport_id": SPORT_ID, "sport_name": "Biathlon", "sport_slug": SPORT_ID, "created_at_utc": timestamp}]
        )
        ...
        for row in frame.itertuples(index=False):
            ...
            participant_prefix = "biathlon_world_team" if participant_type == "team" else "biathlon_world_athlete"
            participant_key_name = participant_name
            if participant_type == "team":
                participant_key_name = f"{participant_name}_{row.team_members}"
            participant_id = (
                f"{participant_prefix}_{self._clean_participant_name_for_id(participant_key_name)}_{country_code}"
            )
            ...
            results_rows.append(
                {
                    ...
                    "medal": str(row.medal),
                    "score_raw": (
                        f"discipline={row.discipline_name};event={row.event_name};event_label={row.event_label};"
                        f"team_members={row.team_members};entry_country={country_code};source_url={row.source_url}"
                    ),
    ...
    def _assert_result_profiles(results_df: pd.DataFrame) -> None:
        ...
        if bad_profiles:
            sample = dict(list(bad_profiles.items())[:30])
            raise RuntimeError(f"Unexpected Biathlon World Championships result profiles: {sample}")
