...
SEED_FILE = "world_karate_championships_top4_seed.csv"
...
ALLOWED_PROFILES = {(1, 2, 3, 3)}
COUNTRY_NAME_OVERRIDES = {
    "ANA": "Individual Neutral Athletes",
    "ENG": "England",
    "IRI": "Iran",
    "KOS": "Kosovo",
    "RKF": "Russian Karate Federation",
    "SCO": "Scotland",
    "TPE": "Chinese Taipei",
    "TUR": "Türkiye",
    "VIE": "Vietnam",
    "WKF1": "World Karate Federation-1",
    "WKF2": "World Karate Federation-2",
    "YUG": "Yugoslavia",
}


class WorldKarateChampionshipsHistoryConnector(Connector):
    id = "world_karate_championships_history"
    name = "World Karate Championships Historical Podiums"
    source_type = "csv"
    license_notes = (
        "Historical podium seed curated from public Wikipedia World Karate Championships annual medalist tables. "
        "Verify downstream redistribution requirements."
    )
    base_url = "https://en.wikipedia.org/wiki/Karate_World_Championships"
    ...
    def source_row(self) -> dict[str, str]:
        return {
            ...
            "license_notes": (
                "Local seed data/raw/karate/world_karate_championships_top4_seed.csv built from Wikipedia annual "
                "World Karate Championships medalist tables. Strict post-2000 scope; senior WKF events only, "
                "excluding Para Karate and the separate team world championship/cup. Expected rank profile: 1,2,3,3."
            ),
            "base_url": self.base_url,
        }

    def _local_seed_path(self) -> Path:
        return Path(__file__).resolve().parents[2] / "data" / "raw" / "karate" / SEED_FILE
    ...
    def fetch(self, season_year: int, out_dir: Path) -> list[Path]:
        ...
        if not local_seed.exists():
            raise RuntimeError(f"Missing local seed for World Karate Championships: {local_seed}")

        frame = pd.read_csv(local_seed)
        if "year" not in frame.columns:
            raise RuntimeError(f"Unsupported World Karate seed format for {local_seed.name}: missing `year`.")
        ...
        if filtered.empty:
            raise RuntimeError(f"No World Karate rows available for year <= {season_year} and year > 2000.")
    ...
    def parse(self, raw_paths: list[Path], season_year: int) -> dict[str, pd.DataFrame]:
        ...
        if not required_cols.issubset(set(frame.columns)):
            raise RuntimeError(f"Unsupported World Karate seed format for {seed_path.name}: {list(frame.columns)}")
        ...
        if (frame["year"] <= 2000).any():
            ...
            raise RuntimeError(f"Post-2000 guard violated for World Karate seed: {offenders}")
        ...
        if bad_profiles:
            ...
            raise RuntimeError(f"Unexpected World Karate rank profiles: {sample}")

        timestamp = utc_now_iso()
        sports_df = pd.DataFrame(
            [{"sport_id": SPORT_ID, "sport_name": "Karate", "sport_slug": SPORT_ID, "created_at_utc": timestamp}]
        )
            ...
            [
                {
                    ...
                    "sport_id": SPORT_ID,
                    "name": "World Karate Championships",
                    "season_year": None,
        ...
        for row in frame.itertuples(index=False):
            ...
            participant_prefix = "world_karate_team" if participant_type == "team" else "world_karate_athlete"
    ...
    def _assert_result_profiles(results_df: pd.DataFrame) -> None:
        ...
        if bad_profiles:
            sample = dict(list(bad_profiles.items())[:30])
            raise RuntimeError(f"Unexpected World Karate result profiles after participant build: {sample}")
