...
SEED_FILE = "world_boxing_championships_top4_seed.csv"
SPORT_ID = "boxing"
...
COMPETITIONS = {
    "iba_mens_world_boxing_championships": "IBA Men's World Boxing Championships",
    "iba_womens_world_boxing_championships": "IBA Women's World Boxing Championships",
    "world_boxing_championships": "World Boxing Championships",
}
COUNTRY_NAME_OVERRIDES = {
    ...
    "SCO": "Scotland",
    "TBF": "Thailand Boxing Federation",
    "TPE": "Chinese Taipei",
    "TUR": "Türkiye",
    "VIE": "Vietnam",
    "WAL": "Wales",
...
class WorldBoxingChampionshipsHistoryConnector(Connector):
    id = "world_boxing_championships_history"
    name = "World Boxing Championships Historical Podiums"
    source_type = "csv"
    license_notes = (
        "Historical podium seed curated from public Wikipedia IBA and World Boxing Championships medalist tables. "
        "Verify downstream redistribution requirements."
    )
    base_url = "https://en.wikipedia.org/wiki/World_Boxing_Championships"
    ...
    def source_row(self) -> dict[str, str]:
        return {
            ...
            "license_notes": (
                "Local seed data/raw/boxing/world_boxing_championships_top4_seed.csv built from Wikipedia annual "
                "IBA men's/women's medalist tables and 2025 World Boxing Championships medalist tables. Strict "
                "post-2000 scope. IBA and World Boxing are kept as separate competitions; expected rank profiles "
                "are 1,2,3,3, with source-official 1,2,3 allowed when only one bronze is listed."
            ),
            "base_url": self.base_url,
        }

    def _local_seed_path(self) -> Path:
        return Path(__file__).resolve().parents[2] / "data" / "raw" / "boxing" / SEED_FILE
    ...
    @staticmethod
    def _event_id(competition_id: str, year: int, gender: str, event_key: str) -> str:
    ...
    def fetch(self, season_year: int, out_dir: Path) -> list[Path]:
        ...
        if not local_seed.exists():
            raise RuntimeError(f"Missing local seed for World Boxing Championships: {local_seed}")

        frame = pd.read_csv(local_seed)
        if "year" not in frame.columns:
            raise RuntimeError(f"Unsupported World Boxing seed format for {local_seed.name}: missing `year`.")
        ...
        if filtered.empty:
            raise RuntimeError(f"No World Boxing rows available for year <= {season_year} and year > 2000.")
    ...
    def parse(self, raw_paths: list[Path], season_year: int) -> dict[str, pd.DataFrame]:
        ...
        if not required_cols.issubset(set(frame.columns)):
            raise RuntimeError(f"Unsupported World Boxing seed format for {seed_path.name}: {list(frame.columns)}")
        ...
        frame = frame.loc[
            ...
            & frame["competition_id"].isin(COMPETITIONS)
        ...
        if (frame["year"] <= 2000).any():
            ...
            raise RuntimeError(f"Post-2000 guard violated for World Boxing seed: {offenders}")
        ...
        frame = frame.drop_duplicates(
            subset=["competition_id", "year", "gender", "event_key", "rank", "participant_name", "country_code"],
            keep="first",
        )
        ...
        if bad_profiles:
            ...
            raise RuntimeError(f"Unexpected World Boxing rank profiles: {sample}")

        timestamp = utc_now_iso()
        sports_df = pd.DataFrame(
            [{"sport_id": SPORT_ID, "sport_name": "Boxing", "sport_slug": SPORT_ID, "created_at_utc": timestamp}]
        )
            ...
            [
                {
                    ...
                    "start_date": comp_frame["event_date"].min(),
                    "end_date": comp_frame["event_date"].max(),
                    "source_id": self.id,
                }
                for competition_id, competition_name in COMPETITIONS.items()
                for comp_frame in [frame.loc[frame["competition_id"] == competition_id]]
        ...
        for competition_id, year, gender, event_key, event_date in (
            frame[["competition_id", "year", "gender", "event_key", "event_date"]]
            .drop_duplicates()
            .sort_values(["competition_id", "year", "gender", "event_key"])
            .itertuples(index=False)
        ):
            events_rows.append(
                {
                    "event_id": self._event_id(str(competition_id), int(year), str(gender), str(event_key)),
                    "competition_id": str(competition_id),
                    "discipline_id": DISCIPLINE_ID,
                    "gender": str(gender),
                    "event_class": "podium_top4",
        ...
        for row in frame.itertuples(index=False):
            ...
            participant_id = (
                f"world_boxing_athlete_{self._clean_participant_name_for_id(participant_name)}_{country_code}"
            )
            ...
            rank = int(row.rank)
            event_id = self._event_id(str(row.competition_id), int(row.year), str(row.gender), str(row.event_key))
            results_rows.append(
                {
                    "event_id": event_id,
                    "participant_id": participant_id,
                    ...
                    "score_raw": (
                        ...
                        f"event_label={row.event_label};source_text={row.source_text};"
    ...
    def _assert_result_profiles(results_df: pd.DataFrame) -> None:
        ...
        if bad_profiles:
            sample = dict(list(bad_profiles.items())[:30])
            raise RuntimeError(f"Unexpected World Boxing result profiles after participant build: {sample}")
