...
SEED_FILE = "fig_artistic_gymnastics_world_championships_top3_seed.csv"
...
DISCIPLINE_IDS = {
    "balance-beam": "artistic-gymnastics-balance-beam",
    "floor-exercise": "artistic-gymnastics-floor-exercise",
    "horizontal-bar": "artistic-gymnastics-horizontal-bar",
    "individual-all-around": "artistic-gymnastics-individual-all-around",
    "parallel-bars": "artistic-gymnastics-parallel-bars",
    "pommel-horse": "artistic-gymnastics-pommel-horse",
    "still-rings": "artistic-gymnastics-still-rings",
    "team": "artistic-gymnastics-team",
    "uneven-bars": "artistic-gymnastics-uneven-bars",
    "vault": "artistic-gymnastics-vault",
}
...
class FigArtisticGymnasticsWorldChampionshipsHistoryConnector(Connector):
    id = "fig_artistic_gymnastics_world_championships_history"
    name = "FIG Artistic Gymnastics World Championships Historical Podiums"
    source_type = "csv"
    license_notes = (
        "Historical podium seed curated from public Wikipedia annual World Artistic Gymnastics Championships "
        "medalist tables. Verify downstream redistribution requirements."
    )
    base_url = "https://en.wikipedia.org/wiki/World_Artistic_Gymnastics_Championships"
    ...
    def source_row(self) -> dict[str, str]:
        return {
            ...
            "license_notes": (
                "Local seed data/raw/gymnastics/fig_artistic_gymnastics_world_championships_top3_seed.csv "
                "built from Wikipedia annual medalist tables. Strict post-2000 scope; 2026 skipped until "
                "medalists are published. Official medal ties are preserved."
            ),
            "base_url": self.base_url,
        }

    def _local_seed_path(self) -> Path:
        return Path(__file__).resolve().parents[2] / "data" / "raw" / "gymnastics" / SEED_FILE
    ...
    def _event_id(year: int, gender: str, discipline_key: str) -> str:
        return f"{COMPETITION_ID}_{year}_{slugify(gender)}_{slugify(discipline_key)}"

    def fetch(self, season_year: int, out_dir: Path) -> list[Path]:
        local_seed = self._local_seed_path()
        if not local_seed.exists():
            raise RuntimeError(f"Missing local seed for FIG Artistic Gymnastics Worlds: {local_seed}")

        frame = pd.read_csv(local_seed)
        if "year" not in frame.columns:
            raise RuntimeError(f"Unsupported FIG artistic gymnastics seed format for {local_seed.name}: missing `year`.")
        ...
        if filtered.empty:
            raise RuntimeError(
                f"No FIG artistic gymnastics rows available for year <= {season_year} and year > 2000."
            )

        years_available = sorted(int(year) for year in filtered["year"].unique().tolist())
        years_with_page_but_no_medals = [year for year in [2026] if 2000 < year <= int(season_year)]
        ...
        self._write_json(
            out_dir / "fetch_meta.json",
            {
                ...
                "years_with_page_but_no_medalists": years_with_page_but_no_medals,
            },
        )
        return [out_file]
    ...
    def parse(self, raw_paths: list[Path], season_year: int) -> dict[str, pd.DataFrame]:
        ...
        if not required_cols.issubset(set(frame.columns)):
            raise RuntimeError(f"Unsupported FIG artistic gymnastics seed format for {seed_path.name}: {list(frame.columns)}")
        ...
        if (frame["year"] <= 2000).any():
            ...
            raise RuntimeError(f"Post-2000 guard violated for FIG artistic gymnastics seed: {offenders}")
        ...
        frame = frame.drop_duplicates(
            subset=["year", "gender", "discipline_key", "rank", "participant_name", "country_code"],
            keep="first",
        )
        ...
        profiles = (
            frame.groupby(["year", "gender", "discipline_key"])["rank"]
        ...
        if bad_profiles:
            ...
            raise RuntimeError(f"Unexpected FIG artistic gymnastics rank profiles: {sample}")

        timestamp = utc_now_iso()
        sports_df = pd.DataFrame(
            [{"sport_id": SPORT_ID, "sport_name": "Gymnastics", "sport_slug": SPORT_ID, "created_at_utc": timestamp}]
        )
        discipline_frame = frame[["discipline_key", "discipline_name"]].drop_duplicates()
        disciplines_df = pd.DataFrame(
            [
                {
                    "discipline_id": DISCIPLINE_IDS[str(row.discipline_key)],
                    "discipline_name": str(row.discipline_name),
                    "discipline_slug": slugify(str(row.discipline_name)),
                    "sport_id": SPORT_ID,
                    "confidence": 1.0,
                    "mapping_source": f"connector_{self.id}",
                    "created_at_utc": timestamp,
                }
                for row in discipline_frame.sort_values("discipline_key").itertuples(index=False)
            ]
        )
            ...
            [
                {
                    ...
                    "sport_id": SPORT_ID,
                    "name": "FIG Artistic Gymnastics World Championships",
                    "season_year": None,
        ...
        for year, gender, discipline_key, event_date in (
            frame[["year", "gender", "discipline_key", "event_date"]]
            .drop_duplicates()
            ...
            events_rows.append(
                {
                    ...
                    "competition_id": COMPETITION_ID,
                    "discipline_id": DISCIPLINE_IDS[str(discipline_key)],
                    "gender": str(gender),
                    "event_class": "podium_top3_with_ties",
        ...
        for row in frame.itertuples(index=False):
            ...
            participant_prefix = "fig_team" if participant_type == "team" else "fig_athlete"
    ...
    def _assert_result_profiles(results_df: pd.DataFrame) -> None:
        ...
        if bad_profiles:
            sample = dict(list(bad_profiles.items())[:30])
            raise RuntimeError(f"Unexpected FIG artistic gymnastics result profiles after participant build: {sample}")
