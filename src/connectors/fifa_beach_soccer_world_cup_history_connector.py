...
SEED_FILE = "fifa_beach_soccer_world_cup_top4_seed.csv"
COMPETITION_ID = "fifa_beach_soccer_world_cup"
SPORT_ID = "football"
DISCIPLINE_ID = "beach-soccer"
...
class FifaBeachSoccerWorldCupHistoryConnector(Connector):
    id = "fifa_beach_soccer_world_cup_history"
    name = "FIFA Beach Soccer World Cup Historical Top 4"
    source_type = "csv"
    license_notes = (
        "Historical top4 seed curated from public Wikipedia FIFA Beach Soccer World Cup tables. "
        "Verify downstream redistribution requirements."
    )
    base_url = "https://en.wikipedia.org/wiki/FIFA_Beach_Soccer_World_Cup"
    ...
    def source_row(self) -> dict[str, str]:
        return {
            ...
            "license_notes": (
                "Local seed data/raw/football/fifa_beach_soccer_world_cup_top4_seed.csv built from "
                "Wikipedia FIFA Beach Soccer World Cup editions table. Strict post-2000 scope; "
    ...
    def _local_seed_path(self) -> Path:
        return Path(__file__).resolve().parents[2] / "data" / "raw" / "football" / SEED_FILE
    ...
    def fetch(self, season_year: int, out_dir: Path) -> list[Path]:
        ...
        if not local_seed.exists():
            raise RuntimeError(f"Missing local seed for FIFA Beach Soccer World Cup: {local_seed}")

        frame = pd.read_csv(local_seed)
        if "year" not in frame.columns:
            raise RuntimeError(
                f"Unsupported FIFA Beach Soccer World Cup seed format for {local_seed.name}: missing `year`."
            )
        ...
        if filtered.empty:
            raise RuntimeError(f"No FIFA Beach Soccer World Cup rows for year <= {season_year} and year > 2000.")
    ...
    def parse(self, raw_paths: list[Path], season_year: int) -> dict[str, pd.DataFrame]:
        ...
        if not required_cols.issubset(set(frame.columns)):
            raise RuntimeError(f"Unsupported FIFA Beach Soccer World Cup seed columns: {list(frame.columns)}")
        ...
        if (frame["year"] <= 2000).any():
            ...
            raise RuntimeError(f"Post-2000 guard violated for FIFA Beach Soccer World Cup seed: {offenders}")
        ...
        if bad_profiles:
            ...
            raise RuntimeError(f"Unexpected FIFA Beach Soccer World Cup rank profiles: {sample}")

        timestamp = utc_now_iso()
        sports_df = pd.DataFrame(
            [{"sport_id": SPORT_ID, "sport_name": "Football", "sport_slug": SPORT_ID, "created_at_utc": timestamp}]
        )
            ...
            results_rows.append(
                {
                    ...
                    "rank": rank,
                    "medal": str(row.medal),
                    "score_raw": f"fifa_beach_soccer_world_cup_final_rank={rank};source_url={row.source_url}",
    ...
    def _assert_result_profiles(results_df: pd.DataFrame) -> None:
        ...
        if bad_profiles:
            sample = dict(list(bad_profiles.items())[:20])
            raise RuntimeError(f"Unexpected FIFA Beach Soccer World Cup result profiles: {sample}")
