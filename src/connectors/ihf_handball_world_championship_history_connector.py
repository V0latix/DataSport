...
COMPETITIONS: dict[str, dict[str, str]] = {
    "men": {
        "seed_file": "ihf_world_men_handball_championship_top4_seed.csv",
        "competition_id": "ihf_handball_world_championship_men",
        "competition_name": "IHF World Men's Handball Championship",
        "discipline_name": "IHF Handball World Championship Men Final Ranking",
        "gender": "men",
    },
    "women": {
        "seed_file": "ihf_world_women_handball_championship_top4_seed.csv",
        "competition_id": "ihf_handball_world_championship_women",
        "competition_name": "IHF Women's Handball World Championship",
        "discipline_name": "IHF Handball World Championship Women Final Ranking",
        "gender": "women",
    },
}


class IhfHandballWorldChampionshipHistoryConnector(Connector):
    id = "ihf_handball_world_championship_history"
    name = "IHF Handball World Championship Historical Results (Men/Women)"
    source_type = "csv"
    license_notes = (
        "Historical top4 seeds curated from public IHF / Wikipedia references. "
        "Verify downstream redistribution requirements."
    )
    base_url = "https://en.wikipedia.org/wiki/World_Men%27s_Handball_Championship"
    ...
    def source_row(self) -> dict[str, str]:
        return {
            ...
            "license_notes": (
                ...
                "data/raw/handball/ihf_world_men_handball_championship_top4_seed.csv and "
                "data/raw/handball/ihf_world_women_handball_championship_top4_seed.csv "
                "(curated from IHF / Wikipedia public information)."
            ),
            "base_url": self.base_url,
        }

    def _local_seed_path(self, seed_file: str) -> Path:
        return Path(__file__).resolve().parents[2] / "data" / "raw" / "handball" / seed_file
    ...
    def fetch(self, season_year: int, out_dir: Path) -> list[Path]:
        ...
        for meta in COMPETITIONS.values():
            ...
            if not local_seed.exists():
                raise RuntimeError(f"Missing local seed for IHF handball world championship history: {local_seed}")
    ...
    def parse(self, raw_paths: list[Path], season_year: int) -> dict[str, pd.DataFrame]:
        ...
        for gender, meta in COMPETITIONS.items():
            ...
            if not required_cols.issubset(set(annual_df.columns)):
                raise RuntimeError(
                    f"Unsupported IHF handball world championship seed format for {seed_file}: {list(annual_df.columns)}"
                )
        ...
        if not parsed_by_gender:
            raise RuntimeError("IHF handball world championship parsing returned no dataset.")

        timestamp = utc_now_iso()
        sport_id = slugify("Handball")
            ...
            disciplines_rows.append(
                {
                    ...
                    "sport_id": sport_id,
                    "confidence": 1.0,
                    "mapping_source": "connector_ihf_handball_world_championship_history",
                    "created_at_utc": timestamp,
                }
            )
                    ...
                    results_rows.append(
                        {
                            ...
                            "rank": rank,
                            "medal": medal,
                            "score_raw": f"ihf_handball_world_championship_final_rank={rank}",
    ...
    def upsert(self, db: SQLiteDB, payload: dict[str, pd.DataFrame]) -> None:
        with db.connect() as conn:
            ...
            conn.execute(
                "DELETE FROM disciplines WHERE mapping_source = 'connector_ihf_handball_world_championship_history'"
            )
            conn.commit()
