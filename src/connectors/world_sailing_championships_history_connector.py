...
SEED_FILE = "world_sailing_championships_top3_seed.csv"
...
DISCIPLINE_NAMES = {
    "sailing-2-4-metre": "2.4 Metre",
    "sailing-470": "470",
    "sailing-49er": "49er",
    "sailing-49er-fx": "49er FX",
    "sailing-elliott-6m": "Elliott 6m",
    "sailing-europe": "Europe",
    "sailing-finn": "Finn",
    "sailing-formula-kite": "Formula Kite",
    "sailing-hansa-303": "Hansa 303",
    "sailing-ilca-6": "ILCA 6",
    "sailing-ilca-7": "ILCA 7",
    "sailing-iqfoil": "iQFoil",
    "sailing-laser": "Laser",
    "sailing-laser-radial": "Laser Radial",
    "sailing-mistral": "Mistral",
    "sailing-nacra-17": "Nacra 17",
    "sailing-rs-venture-connect": "RS Venture Connect",
    "sailing-rsx": "RS:X",
    "sailing-star": "Star",
    "sailing-tornado": "Tornado",
    "sailing-yngling": "Yngling",
}
...
class WorldSailingChampionshipsHistoryConnector(Connector):
    id = "world_sailing_championships_history"
    name = "Sailing World Championships Historical Class Podiums"
    source_type = "csv"
    license_notes = (
        "Historical class podium seed curated from public Wikipedia Sailing World Championships medalist tables. "
        "Verify downstream redistribution requirements."
    )
    base_url = "https://en.wikipedia.org/wiki/Sailing_World_Championships"
    ...
    def source_row(self) -> dict[str, str]:
        return {
            ...
            "license_notes": (
                "Local seed data/raw/sailing/world_sailing_championships_top3_seed.csv built from Wikipedia "
                "event medalist tables for combined ISAF/Sailing World Championships editions. Strict post-2000 "
                "scope; standalone annual class championships outside the combined Worlds are excluded."
            ),
            "base_url": self.base_url,
        }

    def _local_seed_path(self) -> Path:
        return Path(__file__).resolve().parents[2] / "data" / "raw" / "sailing" / SEED_FILE
    ...
    @staticmethod
    def _event_id(year: int, class_key: str, gender: str) -> str:
        return f"{COMPETITION_ID}_{year}_{slugify(class_key)}_{slugify(gender)}"

    def fetch(self, season_year: int, out_dir: Path) -> list[Path]:
        local_seed = self._local_seed_path()
        if not local_seed.exists():
            raise RuntimeError(f"Missing local seed for Sailing World Championships: {local_seed}")

        frame = pd.read_csv(local_seed)
        if "year" not in frame.columns:
            raise RuntimeError(f"Unsupported Sailing Worlds seed format for {local_seed.name}: missing `year`.")
        ...
        if filtered.empty:
            raise RuntimeError(f"No Sailing Worlds rows available for year <= {season_year} and year > 2000.")
    ...
    def parse(self, raw_paths: list[Path], season_year: int) -> dict[str, pd.DataFrame]:
        ...
        if not required_cols.issubset(set(frame.columns)):
            raise RuntimeError(f"Unsupported Sailing Worlds seed format for {seed_path.name}: {list(frame.columns)}")
        ...
        frame["class_key"] = frame["class_key"].fillna("").astype(str).str.strip().str.lower()
        ...
        frame = frame.loc[
            ...
            & frame["discipline_key"].isin(DISCIPLINE_NAMES)
        ...
        if (frame["year"] <= 2000).any():
            ...
            raise RuntimeError(f"Post-2000 guard violated for Sailing Worlds seed: {offenders}")
        ...
        frame = frame.drop_duplicates(
            subset=["year", "class_key", "gender", "rank", "participant_name", "country_code"],
            keep="first",
        )
        frame = frame.sort_values(["year", "class_key", "gender", "rank", "country_code", "participant_name"]).reset_index(
            drop=True
        )
        ...
        if bad_profiles:
            ...
            raise RuntimeError(f"Unexpected Sailing Worlds rank profiles: {sample}")
            ...
            [
                {
                    ...
                    "sport_id": "sailing",
                    "confidence": 1.0,
                    "mapping_source": "connector_world_sailing_championships_history",
                    "created_at_utc": timestamp,
                }
                for discipline_id, discipline_name in sorted(DISCIPLINE_NAMES.items())
                if discipline_id in set(frame["discipline_key"].tolist())
            ]
        )
            ...
            [
                {
                    ...
                    "sport_id": "sailing",
                    "name": "Sailing World Championships",
                    "season_year": None,
        ...
        for year, class_key, gender, discipline_key, event_date in (
            frame[["year", "class_key", "gender", "discipline_key", "event_date"]]
            .drop_duplicates()
            .sort_values(["year", "class_key", "gender"])
            .itertuples(index=False)
        ):
            events_rows.append(
                {
                    "event_id": self._event_id(int(year), str(class_key), str(gender)),
                    "competition_id": COMPETITION_ID,
                    "discipline_id": str(discipline_key),
                    "gender": str(gender),
                    "event_class": "podium_top3_class",
        ...
        for row in frame.itertuples(index=False):
            ...
            participant_id = f"sailing_entry_{self._clean_participant_name_for_id(participant_name)}_{country_code}"
            ...
            rank = int(row.rank)
            event_id = self._event_id(int(row.year), str(row.class_key), str(row.gender))
            results_rows.append(
                {
                    "event_id": event_id,
                    "participant_id": participant_id,
                    ...
                    "medal": str(row.medal),
                    "score_raw": (
                        f"class={row.class_key};event_name={row.event_name};"
    ...
    def _assert_result_profiles(results_df: pd.DataFrame) -> None:
        ...
        if bad_profiles:
            sample = dict(list(bad_profiles.items())[:30])
            raise RuntimeError(f"Unexpected Sailing Worlds result profiles after participant build: {sample}")
