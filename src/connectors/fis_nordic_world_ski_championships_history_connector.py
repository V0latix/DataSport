...
SEED_FILE = "fis_nordic_world_ski_championships_top3_seed.csv"
...
class FisNordicWorldSkiChampionshipsHistoryConnector(Connector):
    id = "fis_nordic_world_ski_championships_history"
    name = "FIS Nordic World Ski Championships Historical Podiums"
    source_type = "csv"
    license_notes = (
        "Historical podium seed curated from public Wikipedia FIS Nordic World Ski Championships medalist "
        "tables. Verify downstream redistribution requirements."
    )
    base_url = "https://en.wikipedia.org/wiki/FIS_Nordic_World_Ski_Championships"
    ...
    def source_row(self) -> dict[str, str]:
        return {
            ...
            "license_notes": (
                "Local seed data/raw/skiing/fis_nordic_world_ski_championships_top3_seed.csv built from "
                "Wikipedia cross-country skiing, ski jumping and nordic combined medalist list tables. "
                "Strict post-2000 scope; official medal ties are preserved."
            ),
            "base_url": self.base_url,
        }

    def _local_seed_path(self) -> Path:
        return Path(__file__).resolve().parents[2] / "data" / "raw" / "skiing" / SEED_FILE
    ...
    @staticmethod
    def _event_id(year: int, discipline_key: str, gender: str, event_key: str) -> str:
        return f"{COMPETITION_ID}_{year}_{slugify(discipline_key)}_{slugify(gender)}_{slugify(event_key)}"

    def fetch(self, season_year: int, out_dir: Path) -> list[Path]:
        local_seed = self._local_seed_path()
        if not local_seed.exists():
            raise RuntimeError(f"Missing local seed for FIS Nordic Worlds: {local_seed}")

        frame = pd.read_csv(local_seed)
        if "year" not in frame.columns:
            raise RuntimeError(f"Unsupported FIS Nordic seed format for {local_seed.name}: missing `year`.")
        ...
        if filtered.empty:
            raise RuntimeError(f"No FIS Nordic rows available for year <= {season_year} and year > 2000.")

        years_available = sorted(int(year) for year in filtered["year"].unique().tolist())
        non_championship_or_unavailable_years = sorted(set(range(2001, int(season_year) + 1)) - set(years_available))
        ...
        self._write_json(
            out_dir / "fetch_meta.json",
            {
                ...
                "non_championship_or_unavailable_years_up_to_requested": non_championship_or_unavailable_years,
            },
        )
        return [out_file]
    ...
    def parse(self, raw_paths: list[Path], season_year: int) -> dict[str, pd.DataFrame]:
        ...
        if not required_cols.issubset(set(frame.columns)):
            raise RuntimeError(f"Unsupported FIS Nordic seed format for {seed_path.name}: {list(frame.columns)}")
        ...
        frame = frame.loc[
            ...
            & frame["discipline_key"].isin(DISCIPLINES)
        ...
        if (frame["year"] <= 2000).any():
            offenders = frame.loc[frame["year"] <= 2000, ["year", "discipline_key", "event_key", "gender"]].head(10)
            raise RuntimeError(f"Post-2000 guard violated for FIS Nordic seed: {offenders.to_dict('records')}")
        ...
        frame = frame.drop_duplicates(
            subset=["year", "discipline_key", "gender", "event_key", "rank", "participant_name", "country_code"],
            keep="first",
        )
        ...
        sports_df = pd.DataFrame(
            [{"sport_id": SPORT_ID, "sport_name": "Skiing", "sport_slug": SPORT_ID, "created_at_utc": timestamp}]
        )
        ...
        for year, discipline_key, gender, event_key, event_date in (
            frame[["year", "discipline_key", "gender", "event_key", "event_date"]]
            .drop_duplicates()
            .sort_values(["year", "discipline_key", "gender", "event_key"])
            .itertuples(index=False)
        ):
            events_rows.append(
                {
                    "event_id": self._event_id(int(year), str(discipline_key), str(gender), str(event_key)),
                    "competition_id": COMPETITION_ID,
        ...
        for row in frame.itertuples(index=False):
            ...
            participant_name = str(row.participant_name).strip() or country_name
            if participant_type == "team":
                participant_id = country_code
            else:
                participant_id = f"{country_code}_{self._clean_participant_name_for_id(participant_name)}"
            ...
            results_rows.append(
                {
                    "event_id": self._event_id(int(row.year), str(row.discipline_key), str(row.gender), str(row.event_key)),
                    "participant_id": participant_id,
                    "rank": rank,
                    "medal": str(row.medal),
                    "score_raw": (
                        f"fis_nordic_worlds_rank={rank};event={row.event_name};team_members={row.team_members};"
    ...
    def _assert_result_profiles(results_df: pd.DataFrame) -> None:
        ...
        if bad_profiles:
            sample = dict(list(bad_profiles.items())[:30])
            raise RuntimeError(f"Unexpected FIS Nordic result profiles: {sample}")
    ...
    def upsert(self, db: SQLiteDB, payload: dict[str, pd.DataFrame]) -> None:
        ...
        if not participants_df.empty:
            participants_df = participants_df.loc[~participants_df["participant_id"].isin(existing_participant_ids)].copy()
