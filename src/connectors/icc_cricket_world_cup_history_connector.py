...
class IccCricketWorldCupHistoryConnector(Connector):
    ...
    def _local_seed_path(self, seed_file: str) -> Path:
        return Path(__file__).resolve().parents[2] / "data" / "raw" / "cricket" / seed_file
    ...
    def fetch(self, season_year: int, out_dir: Path) -> list[Path]:
        ...
        for meta in COMPETITIONS.values():
            ...
            if not local_seed.exists():
                raise RuntimeError(f"Missing local seed for ICC cricket world cup history: {local_seed}")
    ...
    def parse(self, raw_paths: list[Path], season_year: int) -> dict[str, pd.DataFrame]:
        ...
        for gender, meta in COMPETITIONS.items():
            ...
            if not required_cols.issubset(set(annual_df.columns)):
                raise RuntimeError(
                    f"Unsupported ICC cricket world cup seed format for {seed_file}: {list(annual_df.columns)}"
                )
        ...
        if not parsed_by_gender:
            raise RuntimeError("ICC cricket world cup parsing returned no dataset.")
            ...
            disciplines_rows.append(
                {
                    ...
                    "sport_id": sport_id,
                    "confidence": 1.0,
                    "mapping_source": "connector_icc_cricket_world_cup_history",
                    "created_at_utc": timestamp,
                }
            )
    ...
    def upsert(self, db: SQLiteDB, payload: dict[str, pd.DataFrame]) -> None:
        with db.connect() as conn:
            ...
            conn.execute("DELETE FROM disciplines WHERE mapping_source = 'connector_icc_cricket_world_cup_history'")
            conn.commit()
