...
SEED_FILE = "bwf_world_championships_top4_by_discipline_seed.csv"
RANK_TO_MEDAL = {1: "gold", 2: "silver", 3: "bronze"}
RANK_TO_POINTS = {1: 10.0, 2: 7.0, 3: 5.0}
DISCIPLINE_CONFIG: dict[str, dict[str, str]] = {
    "mens_singles": {
        "discipline_id": "badminton_mens-singles",
        "discipline_name": "Men's singles",
        "gender": "men",
        "participant_type": "athlete",
    },
    "womens_singles": {
        "discipline_id": "badminton_womens-singles",
        "discipline_name": "Women's singles",
        "gender": "women",
        "participant_type": "athlete",
    },
    "mens_doubles": {
        "discipline_id": "badminton_mens-doubles",
        "discipline_name": "Men's doubles",
        "gender": "men",
        "participant_type": "team",
    },
    "womens_doubles": {
        "discipline_id": "badminton_womens-doubles",
        "discipline_name": "Women's doubles",
        "gender": "women",
        "participant_type": "team",
    },
    "mixed_doubles": {
        "discipline_id": "badminton_mixed-doubles",
...
class BwfWorldChampionshipsHistoryConnector(Connector):
    id = "bwf_world_championships_history"
    name = "BWF World Championships Historical Top 4 by Discipline"
    source_type = "csv"
    license_notes = (
        "Historical top-4 seeds curated from public BWF medalist information. "
        "Verify downstream redistribution requirements."
    )
    base_url = "https://en.wikipedia.org/wiki/List_of_BWF_World_Championships_medalists"
    ...
    def source_row(self) -> dict[str, str]:
        return {
            ...
            "license_notes": (
                "Historical top-4 by discipline seeds from local file "
                "data/raw/badminton/bwf_world_championships_top4_by_discipline_seed.csv "
                "(curated from public BWF medalist pages)."
            ),
    ...
    def fetch(self, season_year: int, out_dir: Path) -> list[Path]:
        ...
        if not local_seed.exists():
            raise RuntimeError(f"Missing local seed for BWF world championships: {local_seed}")
    ...
    def _event_id(year: int, discipline_key: str) -> str:
        return f"bwf_world_championships_{year}_{discipline_key}"
    ...
    def parse(self, raw_paths: list[Path], season_year: int) -> dict[str, pd.DataFrame]:
        ...
        if not required_cols.issubset(set(frame.columns)):
            raise RuntimeError(f"Unsupported BWF seed format: {list(frame.columns)}")
        ...
        if frame.empty:
            raise RuntimeError("BWF world championships parsing returned no rows.")

        event_sizes = frame.groupby(["year", "discipline_key"]).size()
        invalid_event_sizes = event_sizes[event_sizes != 4]
        if not invalid_event_sizes.empty:
            raise RuntimeError(
                f"Expected 4 results per BWF event (1/2/3/3); got: {invalid_event_sizes.to_dict()}"
            )
        ...
        if invalid_profiles:
            raise RuntimeError(f"Unexpected BWF rank profiles (expected 1/2/3/3): {invalid_profiles}")
            ...
            disciplines_rows.append(
                {
                    ...
                    "sport_id": "badminton",
                    "confidence": 1.0,
                    "mapping_source": "connector_bwf_world_championships_history",
                    "created_at_utc": timestamp,
                }
            )
        ...
        competitions_rows = [
            {
                "competition_id": "bwf_world_championships",
                "sport_id": "badminton",
        ...
        for row in frame.itertuples(index=False):
            ...
            participant_prefix = "athlete" if participant_type == "athlete" else "pair"
            ...
            results_rows.append(
                {
                    ...
                    "rank": int(row.rank),
                    "medal": str(row.medal),
                    "score_raw": f"discipline={discipline_key};rank={int(row.rank)}",
                    "points_awarded": RANK_TO_POINTS.get(int(row.rank)),
                }
            )
        ...
        with db.connect() as conn:
            ...
            if connector_discipline_ids:
                ...
                conn.execute(
                    ...
                    WHERE mapping_source = 'connector_bwf_world_championships_history'
