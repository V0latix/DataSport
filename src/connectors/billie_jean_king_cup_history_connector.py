...
SEED_FILE = "billie_jean_king_cup_finalists_seed.csv"
COMPETITION_ID = "billie_jean_king_cup"
SPORT_ID = "tennis"
DISCIPLINE_ID = "tennis"
RANK_TO_MEDAL = {1: "gold", 2: "silver"}
RANK_TO_POINTS = {1: 10.0, 2: 7.0}
COUNTRY_NAME_OVERRIDES = {
    "GER": "Germany",
    "RTF": "Russian Tennis Federation",
    "SUI": "Switzerland",
}


class BillieJeanKingCupHistoryConnector(Connector):
    id = "billie_jean_king_cup_history"
    name = "Billie Jean King Cup Historical Finalists"
    source_type = "csv"
    license_notes = (
        "Historical finalists seed curated from public Wikipedia List of Billie Jean King Cup champions table. "
        "Verify downstream redistribution requirements."
    )
    base_url = "https://en.wikipedia.org/wiki/List_of_Billie_Jean_King_Cup_champions"
    ...
    def source_row(self) -> dict[str, str]:
        return {
            ...
            "license_notes": (
                "Local seed data/raw/tennis/billie_jean_king_cup_finalists_seed.csv built from Wikipedia "
                "List of Billie Jean King Cup champions. Strict post-2000 scope; the competition has no "
                "third-place match, so only finalists are stored with rank profile 1,2. RTF is preserved "
                "as source-domain code."
            ),
            "base_url": self.base_url,
        }

    def _local_seed_path(self) -> Path:
        return Path(__file__).resolve().parents[2] / "data" / "raw" / "tennis" / SEED_FILE
    ...
    def fetch(self, season_year: int, out_dir: Path) -> list[Path]:
        ...
        if not local_seed.exists():
            raise RuntimeError(f"Missing local seed for Billie Jean King Cup: {local_seed}")

        frame = pd.read_csv(local_seed)
        if "year" not in frame.columns:
            raise RuntimeError(f"Unsupported Billie Jean King Cup seed format for {local_seed.name}: missing `year`.")
        ...
        if filtered.empty:
            raise RuntimeError(f"No Billie Jean King Cup rows for year <= {season_year} and year > 2000.")
        ...
        self._write_json(
            out_dir / "fetch_meta.json",
            {
                ...
                "source_note": "The source labels one edition as 2020-21; it is normalized to event year 2021.",
            },
        )
        return [out_file]
    ...
    def parse(self, raw_paths: list[Path], season_year: int) -> dict[str, pd.DataFrame]:
        ...
        if not required_cols.issubset(set(frame.columns)):
            raise RuntimeError(f"Unsupported Billie Jean King Cup seed columns: {list(frame.columns)}")
        ...
        if (frame["year"] <= 2000).any():
            ...
            raise RuntimeError(f"Post-2000 guard violated for Billie Jean King Cup seed: {offenders}")

        frame["medal"] = frame.apply(
            lambda row: row["medal"] if row["medal"] in {"gold", "silver"} else RANK_TO_MEDAL[int(row["rank"])],
            axis=1,
        )
        ...
        sports_df = pd.DataFrame(
            [{"sport_id": SPORT_ID, "sport_name": "Tennis", "sport_slug": SPORT_ID, "created_at_utc": timestamp}]
        )
            ...
            results_rows.append(
                {
                    ...
                    "medal": str(row.medal),
                    "score_raw": (
                        f"billie_jean_king_cup_final_rank={rank};score={row.score};"
                        f"source_year={row.source_year};source_url={row.source_url}"
                    ),
    ...
    def _assert_result_profiles(results_df: pd.DataFrame) -> None:
        ...
        if bad_profiles:
            sample = dict(list(bad_profiles.items())[:20])
            raise RuntimeError(f"Unexpected Billie Jean King Cup result profiles: {sample}")
