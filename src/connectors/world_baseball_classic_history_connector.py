...
SEED_FILE = "world_baseball_classic_top4_seed.csv"
...
class WorldBaseballClassicHistoryConnector(Connector):
    id = "world_baseball_classic_history"
    name = "World Baseball Classic Historical Top 4"
    source_type = "csv"
    license_notes = (
        "Historical top4 seed curated from public Wikipedia World Baseball Classic tables. "
        "Verify downstream redistribution requirements."
    )
    base_url = "https://en.wikipedia.org/wiki/World_Baseball_Classic"
    ...
    def source_row(self) -> dict[str, str]:
        return {
            ...
            "license_notes": (
                "Local seed data/raw/baseball/world_baseball_classic_top4_seed.csv built from "
                "Wikipedia World Baseball Classic editions table. Strict post-2000 scope; "
    ...
    def fetch(self, season_year: int, out_dir: Path) -> list[Path]:
        ...
        if not local_seed.exists():
            raise RuntimeError(f"Missing local seed for World Baseball Classic: {local_seed}")

        frame = pd.read_csv(local_seed)
        if "year" not in frame.columns:
            raise RuntimeError(f"Unsupported World Baseball Classic seed format for {local_seed.name}: missing `year`.")
        ...
        if filtered.empty:
            raise RuntimeError(f"No World Baseball Classic rows for year <= {season_year} and year > 2000.")
    ...
    def parse(self, raw_paths: list[Path], season_year: int) -> dict[str, pd.DataFrame]:
        ...
        if not required_cols.issubset(set(frame.columns)):
            raise RuntimeError(f"Unsupported World Baseball Classic seed columns: {list(frame.columns)}")
        ...
        if (frame["year"] <= 2000).any():
            ...
            raise RuntimeError(f"Post-2000 guard violated for World Baseball Classic seed: {offenders}")
        ...
        if bad_profiles:
            ...
            raise RuntimeError(f"Unexpected World Baseball Classic rank profiles: {sample}")

        timestamp = utc_now_iso()
        sports_df = pd.DataFrame(
            [{"sport_id": SPORT_ID, "sport_name": "Baseball", "sport_slug": SPORT_ID, "created_at_utc": timestamp}]
        )
            ...
            results_rows.append(
                {
                    ...
                    "rank": rank,
                    "medal": str(row.medal),
                    "score_raw": f"world_baseball_classic_final_rank={rank};source_url={row.source_url}",
    ...
    def _assert_result_profiles(results_df: pd.DataFrame) -> None:
        ...
        if bad_profiles:
            sample = dict(list(bad_profiles.items())[:20])
            raise RuntimeError(f"Unexpected World Baseball Classic result profiles: {sample}")
