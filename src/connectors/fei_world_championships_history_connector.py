...
SEED_FILE = "fei_world_championships_top3_seed.csv"
...
class FeiWorldChampionshipsHistoryConnector(Connector):
    id = "fei_world_championships_history"
    name = "FEI World Championships Historical Podiums"
    source_type = "csv"
    license_notes = (
        "Historical podium seed curated from public Wikipedia FEI World Equestrian Games and FEI World "
        "Championships medalist tables. Verify downstream redistribution requirements."
    )
    base_url = "https://en.wikipedia.org/wiki/2022_FEI_World_Championships"
    ...
    def source_row(self) -> dict[str, str]:
        return {
            ...
            "license_notes": (
                "Local seed data/raw/equestrian/fei_world_championships_top3_seed.csv built from Wikipedia "
                "FEI World Equestrian Games medalist tables for 2002-2018 and the 2022 FEI World Championships "
                "medalist table. Strict post-2000 scope; cancelled/abandoned events are excluded."
            ),
            "base_url": self.base_url,
        }

    def _local_seed_path(self) -> Path:
        return Path(__file__).resolve().parents[2] / "data" / "raw" / "equestrian" / SEED_FILE
    ...
    def _clean_participant_name_for_id(name: str) -> str:
        ...
        normalized = re.sub(r"\s+", "_", ascii_name.strip())
        normalized = re.sub(r"[^0-9A-Za-z_-]", "", normalized)
        return normalized or slugify(str(name))

    @staticmethod
    def _event_id(year: int, discipline_key: str, event_key: str, gender: str) -> str:
        return f"{COMPETITION_ID}_{year}_{slugify(discipline_key)}_{slugify(event_key)}_{slugify(gender)}"

    def fetch(self, season_year: int, out_dir: Path) -> list[Path]:
        local_seed = self._local_seed_path()
        if not local_seed.exists():
            raise RuntimeError(f"Missing local seed for FEI World Championships: {local_seed}")

        frame = pd.read_csv(local_seed)
        if "year" not in frame.columns:
            raise RuntimeError(f"Unsupported FEI seed format for {local_seed.name}: missing `year`.")
        ...
        if filtered.empty:
            raise RuntimeError(f"No FEI World Championships rows available for year <= {season_year} and year > 2000.")
    ...
    def parse(self, raw_paths: list[Path], season_year: int) -> dict[str, pd.DataFrame]:
        ...
        if not required_cols.issubset(set(frame.columns)):
            raise RuntimeError(f"Unsupported FEI seed format for {seed_path.name}: {list(frame.columns)}")
        ...
        if (frame["year"] <= 2000).any():
            ...
            raise RuntimeError(f"Post-2000 guard violated for FEI seed: {offenders}")
        ...
        frame = frame.drop_duplicates(
            subset=["year", "discipline_key", "event_key", "gender", "rank", "participant_name", "country_code"],
            keep="first",
        )
        frame = frame.sort_values(
            ["year", "discipline_key", "event_key", "gender", "rank", "country_code", "participant_name"]
        ...
        if bad_profiles:
            ...
            raise RuntimeError(f"Unexpected FEI rank profiles: {sample}")
            ...
            [
                {
                    ...
                    "sport_id": "equestrian",
                    "confidence": 1.0,
                    "mapping_source": "connector_fei_world_championships_history",
                    "created_at_utc": timestamp,
                }
        ...
        event_keys = ["year", "discipline_key", "event_key", "event_name", "gender", "event_date"]
        for year, discipline_key, event_key, event_name, gender, event_date in (
            frame[event_keys].drop_duplicates().sort_values(["year", "discipline_key", "event_key", "gender"]).itertuples(index=False)
        ):
            events_rows.append(
                {
                    "event_id": self._event_id(int(year), str(discipline_key), str(event_key), str(gender)),
                    "competition_id": COMPETITION_ID,
        ...
        for row in frame.itertuples(index=False):
            ...
            prefix = "equestrian_team" if participant_type == "team" else "equestrian_athlete"
            participant_id = f"{prefix}_{self._clean_participant_name_for_id(participant_name)}_{country_code}"
            ...
            rank = int(row.rank)
            event_id = self._event_id(int(row.year), str(row.discipline_key), str(row.event_key), str(row.gender))
            results_rows.append(
                {
                    "event_id": event_id,
                    "participant_id": participant_id,
                    ...
                    "medal": str(row.medal),
                    "score_raw": (
                        f"edition={row.edition_name};event={row.event_name};discipline={row.discipline_key};"
    ...
    def _assert_result_profiles(results_df: pd.DataFrame) -> None:
        ...
        if bad_profiles:
            sample = dict(list(bad_profiles.items())[:30])
            raise RuntimeError(f"Unexpected FEI result profiles after participant build: {sample}")
