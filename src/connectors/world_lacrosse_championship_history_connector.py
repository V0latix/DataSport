...
SEED_FILE = "world_lacrosse_championship_top4_seed.csv"
...
class WorldLacrosseChampionshipHistoryConnector(Connector):
    id = "world_lacrosse_championship_history"
    name = "World Lacrosse Championship Historical Top 4"
    source_type = "csv"
    license_notes = (
        "Historical top4 seed curated from public Wikipedia World Lacrosse Men's Championship tables. "
        "Verify downstream redistribution requirements."
    )
    base_url = "https://en.wikipedia.org/wiki/World_Lacrosse_Men%27s_Championship"
    ...
    def source_row(self) -> dict[str, str]:
        return {
            ...
            "license_notes": (
                "Local seed data/raw/lacrosse/world_lacrosse_championship_top4_seed.csv built from "
                "Wikipedia World Lacrosse Men's Championship appearances table. Strict post-2000 scope; "
                "stores men's national-team top4 final standings with rank profile 1,2,3,4."
            ),
    ...
    def fetch(self, season_year: int, out_dir: Path) -> list[Path]:
        ...
        if not local_seed.exists():
            raise RuntimeError(f"Missing local seed for World Lacrosse Championship: {local_seed}")

        frame = pd.read_csv(local_seed)
        if "year" not in frame.columns:
            raise RuntimeError(f"Unsupported World Lacrosse seed format for {local_seed.name}: missing `year`.")
        ...
        if filtered.empty:
            raise RuntimeError(f"No World Lacrosse rows for year <= {season_year} and year > 2000.")
    ...
    def parse(self, raw_paths: list[Path], season_year: int) -> dict[str, pd.DataFrame]:
        ...
        if not required_cols.issubset(set(frame.columns)):
            raise RuntimeError(f"Unsupported World Lacrosse seed columns: {list(frame.columns)}")
        ...
        if (frame["year"] <= 2000).any():
            ...
            raise RuntimeError(f"Post-2000 guard violated for World Lacrosse seed: {offenders}")
        ...
        if bad_profiles:
            ...
            raise RuntimeError(f"Unexpected World Lacrosse rank profiles: {sample}")
            ...
            [
                {
                    ...
                    "sport_id": SPORT_ID,
                    "name": "World Lacrosse Championship",
                    "season_year": None,
            ...
            results_rows.append(
                {
                    ...
                    "rank": rank,
                    "medal": str(row.medal),
                    "score_raw": f"world_lacrosse_championship_final_rank={rank};source_url={row.source_url}",
    ...
    def _assert_result_profiles(results_df: pd.DataFrame) -> None:
        ...
        if bad_profiles:
            sample = dict(list(bad_profiles.items())[:20])
            raise RuntimeError(f"Unexpected World Lacrosse result profiles: {sample}")
