...
SEED_FILE = "world_archery_championships_top3_seed.csv"
...
DISCIPLINE_NAMES = {
    "archery-recurve": "Recurve",
    "archery-compound": "Compound",
}
...
class WorldArcheryChampionshipsHistoryConnector(Connector):
    id = "world_archery_championships_history"
    name = "World Archery Championships Historical Podiums"
    source_type = "csv"
    license_notes = (
        "Historical podium seed curated from public Wikipedia World Archery Championships medalist tables. "
        "Verify downstream redistribution requirements."
    )
    base_url = "https://en.wikipedia.org/wiki/World_Archery_Championships"
    ...
    def source_row(self) -> dict[str, str]:
        return {
            ...
            "license_notes": (
                "Local seed data/raw/archery/world_archery_championships_top3_seed.csv built from Wikipedia "
                "annual World Archery Championships medal summary tables. Strict post-2000 scope; "
                "indoor, field, youth and para championships excluded. Non-ISO source code RAF is used for "
                "Russian Archery Federation in 2021."
            ),
            "base_url": self.base_url,
        }

    def _local_seed_path(self) -> Path:
        return Path(__file__).resolve().parents[2] / "data" / "raw" / "archery" / SEED_FILE
    ...
    def _event_id(year: int, discipline_key: str, event_key: str, gender: str) -> str:
        discipline_short = str(discipline_key).replace("archery-", "")
        return f"{COMPETITION_ID}_{year}_{slugify(discipline_short)}_{slugify(event_key)}_{slugify(gender)}"

    def fetch(self, season_year: int, out_dir: Path) -> list[Path]:
        local_seed = self._local_seed_path()
        if not local_seed.exists():
            raise RuntimeError(f"Missing local seed for World Archery Championships: {local_seed}")

        frame = pd.read_csv(local_seed)
        if "year" not in frame.columns:
            raise RuntimeError(f"Unsupported World Archery seed format for {local_seed.name}: missing `year`.")
        ...
        if filtered.empty:
            raise RuntimeError(f"No World Archery rows available for year <= {season_year} and year > 2000.")
    ...
    def parse(self, raw_paths: list[Path], season_year: int) -> dict[str, pd.DataFrame]:
        ...
        if not required_cols.issubset(set(frame.columns)):
            raise RuntimeError(f"Unsupported World Archery seed format for {seed_path.name}: {list(frame.columns)}")
        ...
        frame = frame.loc[
            ...
            & frame["event_key"].isin(["individual", "team"])
        ...
        if (frame["year"] <= 2000).any():
            ...
            raise RuntimeError(f"Post-2000 guard violated for World Archery seed: {offenders}")
        ...
        if bad_profiles:
            ...
            raise RuntimeError(f"Unexpected World Archery rank profiles: {sample}")

        timestamp = utc_now_iso()
        sports_df = pd.DataFrame(
            [{"sport_id": "archery", "sport_name": "Archery", "sport_slug": "archery", "created_at_utc": timestamp}]
        )
            ...
            [
                {
                    ...
                    "sport_id": "archery",
                    "confidence": 1.0,
                    "mapping_source": "connector_world_archery_championships_history",
        ...
        for year, discipline_key, event_key, gender, event_date in (
            frame[["year", "discipline_key", "event_key", "gender", "event_date"]]
            .drop_duplicates()
            .sort_values(["year", "discipline_key", "event_key", "gender"])
        ...
        for row in frame.itertuples(index=False):
            ...
            participant_prefix = "archery_team" if participant_type == "team" else "archery_athlete"
            ...
            results_rows.append(
                {
                    ...
                    "medal": str(row.medal),
                    "score_raw": (
                        f"discipline={row.discipline_key};event={row.event_name};"
    ...
    def _assert_result_profiles(results_df: pd.DataFrame) -> None:
        ...
        if bad_profiles:
            sample = dict(list(bad_profiles.items())[:30])
            raise RuntimeError(f"Unexpected World Archery result profiles after participant build: {sample}")
