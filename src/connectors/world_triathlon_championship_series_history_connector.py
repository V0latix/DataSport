...
SEED_FILE = "world_triathlon_championship_series_top3_seed.csv"
...
class WorldTriathlonChampionshipSeriesHistoryConnector(Connector):
    id = "world_triathlon_championship_series_history"
    name = "World Triathlon Championship Series Historical Final Standings"
    source_type = "csv"
    license_notes = (
        "Historical WTCS top-three seed curated from public Wikipedia annual World Triathlon Championship Series "
        "and ITU World Triathlon Series standings tables. Verify downstream redistribution requirements."
    )
    base_url = "https://en.wikipedia.org/wiki/World_Triathlon_Championship_Series"
    ...
    def source_row(self) -> dict[str, str]:
        return {
            ...
            "license_notes": (
                "Local seed data/raw/triathlon/world_triathlon_championship_series_top3_seed.csv built from "
                "Wikipedia annual WTCS/ITU World Triathlon Series final standings tables. Scope is the "
                "Championship Series era from 2009 onward; the 2020 COVID-disrupted single-race world "
                "championship classification is included from its annual series page."
            ),
            "base_url": self.base_url,
        }

    def _local_seed_path(self) -> Path:
        return Path(__file__).resolve().parents[2] / "data" / "raw" / "triathlon" / SEED_FILE
    ...
    def _event_id(year: int, gender: str) -> str:
        return f"{COMPETITION_ID}_{year}_{slugify(gender)}"

    def fetch(self, season_year: int, out_dir: Path) -> list[Path]:
        local_seed = self._local_seed_path()
        if not local_seed.exists():
            raise RuntimeError(f"Missing local seed for World Triathlon Championship Series: {local_seed}")

        frame = pd.read_csv(local_seed)
        if "year" not in frame.columns:
            raise RuntimeError(f"Unsupported WTCS seed format for {local_seed.name}: missing `year`.")
        ...
        if filtered.empty:
            raise RuntimeError(f"No WTCS rows available for year <= {season_year} and year > 2000.")

        years_available = sorted(int(year) for year in filtered["year"].unique().tolist())
        missing_years = sorted(set(range(2009, int(season_year) + 1)) - set(years_available))
        ...
        self._write_json(
            out_dir / "fetch_meta.json",
            {
                ...
                "missing_years_series_era_up_to_requested": missing_years,
            },
        )
        return [out_file]
    ...
    def parse(self, raw_paths: list[Path], season_year: int) -> dict[str, pd.DataFrame]:
        ...
        if not required_cols.issubset(set(frame.columns)):
            raise RuntimeError(f"Unsupported WTCS seed format for {seed_path.name}: {list(frame.columns)}")
        ...
        frame = frame.loc[
            ...
            & (frame["discipline_key"] == "triathlon")
            & (frame["event_key"] == "elite_final_standings")
        ...
        if (frame["year"] <= 2000).any():
            ...
            raise RuntimeError(f"Post-2000 guard violated for WTCS seed: {offenders}")
        ...
        if bad_profiles:
            ...
            raise RuntimeError(f"Unexpected WTCS rank profiles: {sample}")

        timestamp = utc_now_iso()
        sports_df = pd.DataFrame(
            [{"sport_id": "triathlon", "sport_name": "Triathlon", "sport_slug": "triathlon", "created_at_utc": timestamp}]
        )
        disciplines_df = pd.DataFrame(
            [
                {
                    "discipline_id": "triathlon",
                    "discipline_name": "Triathlon",
                    "discipline_slug": "triathlon",
                    "sport_id": "triathlon",
                    "confidence": 1.0,
                    "mapping_source": "connector_world_triathlon_championship_series_history",
        ...
        for row in frame.itertuples(index=False):
            ...
            participant_id = f"triathlon_athlete_{self._clean_participant_name_for_id(participant_name)}_{country_code}"
            ...
            results_rows.append(
                {
                    ...
                    "medal": str(row.medal),
                    "score_raw": (
                        f"{row.score_label}={row.score_value};source_note={row.source_note};"
    ...
    def _assert_result_profiles(results_df: pd.DataFrame) -> None:
        ...
        if bad_profiles:
            sample = dict(list(bad_profiles.items())[:30])
            raise RuntimeError(f"Unexpected WTCS result profiles after participant build: {sample}")
