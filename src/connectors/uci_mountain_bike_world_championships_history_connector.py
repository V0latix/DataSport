...
SEED_FILE = "uci_mountain_bike_world_championships_top3_seed.csv"
...
class UciMountainBikeWorldChampionshipsHistoryConnector(Connector):
    id = "uci_mountain_bike_world_championships_history"
    name = "UCI Mountain Bike World Championships Historical Elite Podiums"
    source_type = "csv"
    license_notes = (
        "Historical elite podium seed curated from public Wikipedia UCI Mountain Bike & Trials World Championships "
        "cross-country and downhill pages. Verify downstream redistribution requirements."
    )
    base_url = "https://en.wikipedia.org/wiki/UCI_Mountain_Bike_%26_Trials_World_Championships"
    ...
    def source_row(self) -> dict[str, str]:
        return {
            ...
            "license_notes": (
                "Local seed data/raw/cycling/uci_mountain_bike_world_championships_top3_seed.csv built from "
                "Wikipedia elite cross-country and downhill podium tables. Strict post-2000 scope; "
                "U23, junior, team relay, trials, marathon, eliminator, e-MTB and four-cross categories excluded."
            ),
    ...
    @staticmethod
    def _event_id(year: int, event_key: str, gender: str) -> str:
        return f"{COMPETITION_ID}_{year}_{slugify(event_key)}_{slugify(gender)}_elite"

    def fetch(self, season_year: int, out_dir: Path) -> list[Path]:
        local_seed = self._local_seed_path()
        if not local_seed.exists():
            raise RuntimeError(f"Missing local seed for UCI Mountain Bike worlds: {local_seed}")

        frame = pd.read_csv(local_seed)
        if "year" not in frame.columns:
            raise RuntimeError(f"Unsupported UCI Mountain Bike seed format for {local_seed.name}: missing `year`.")
        ...
        if filtered.empty:
            raise RuntimeError(f"No UCI Mountain Bike rows available for year <= {season_year} and year > 2000.")
    ...
    def parse(self, raw_paths: list[Path], season_year: int) -> dict[str, pd.DataFrame]:
        ...
        if not required_cols.issubset(set(frame.columns)):
            raise RuntimeError(f"Unsupported UCI Mountain Bike seed format for {seed_path.name}: {list(frame.columns)}")
        ...
        frame["event_key"] = frame["event_key"].fillna("").astype(str).str.strip().str.lower()
        ...
        allowed_disciplines = {row["discipline_id"] for row in DISCIPLINE_ROWS}
        ...
        frame = frame.loc[
            ...
            & frame["discipline_key"].isin(allowed_disciplines)
            & frame["event_key"].isin(["xco", "downhill"])
        ...
        if (frame["year"] <= 2000).any():
            ...
            raise RuntimeError(f"Post-2000 guard violated for UCI Mountain Bike seed: {offenders}")
        ...
        frame = frame.drop_duplicates(
            subset=["year", "event_key", "gender", "rank", "participant_name", "country_code"],
            keep="first",
        )
        frame = frame.sort_values(["year", "event_key", "gender", "rank", "country_code", "participant_name"]).reset_index(
            drop=True
        )
        ...
        if bad_profiles:
            ...
            raise RuntimeError(f"Unexpected UCI Mountain Bike rank profiles: {sample}")
            ...
            [
                {
                    **row,
                    "sport_id": "cycling",
                    "confidence": 1.0,
                    "mapping_source": "connector_uci_mountain_bike_world_championships_history",
                    "created_at_utc": timestamp,
                }
            ...
            [
                {
                    ...
                    "sport_id": "cycling",
                    "name": "UCI Mountain Bike World Championships",
                    "season_year": None,
        ...
        for year, event_key, gender, discipline_key, event_date in (
            frame[["year", "event_key", "gender", "discipline_key", "event_date"]]
            .drop_duplicates()
            .sort_values(["year", "event_key", "gender"])
            .itertuples(index=False)
        ):
            events_rows.append(
                {
                    "event_id": self._event_id(int(year), str(event_key), str(gender)),
                    "competition_id": COMPETITION_ID,
        ...
        for row in frame.itertuples(index=False):
            ...
            rank = int(row.rank)
            event_id = self._event_id(int(row.year), str(row.event_key), str(row.gender))
            results_rows.append(
                {
                    "event_id": event_id,
                    "participant_id": participant_id,
                    ...
                    "medal": str(row.medal),
                    "score_raw": (
                        f"discipline={row.discipline_key};event_key={row.event_key};"
                        f"category=elite;source_url={row.source_url}"
                    ),
    ...
    def _assert_result_profiles(results_df: pd.DataFrame) -> None:
        ...
        if bad_profiles:
            sample = dict(list(bad_profiles.items())[:30])
            raise RuntimeError(f"Unexpected UCI Mountain Bike result profiles after participant remap: {sample}")
