...
SEED_FILE = "world_curling_championships_top3_seed.csv"
SPORT_ID = "curling"
DISCIPLINE_ID = "curling"
COMPETITION_IDS = {
...
class WorldCurlingChampionshipsHistoryConnector(Connector):
    id = "world_curling_championships_history"
    name = "World Curling Championships Historical Podiums"
    source_type = "csv"
    license_notes = (
        "Historical podium seed curated from public Wikipedia World Curling Championships tables. "
        "Verify downstream redistribution requirements."
    )
    base_url = "https://en.wikipedia.org/wiki/World_Curling_Championships"
    ...
    def source_row(self) -> dict[str, str]:
        return {
            ...
            "license_notes": (
                "Local seed data/raw/curling/world_curling_championships_top3_seed.csv built from Wikipedia "
                "World Curling Championships annual medal tables. Strict post-2000 scope; wheelchair events "
                "excluded; Scotland/England/Switzerland/Germany codes are preserved as source-domain national "
                "team codes where relevant."
            ),
            "base_url": self.base_url,
        }

    def _local_seed_path(self) -> Path:
        return Path(__file__).resolve().parents[2] / "data" / "raw" / "curling" / SEED_FILE
    ...
    def fetch(self, season_year: int, out_dir: Path) -> list[Path]:
        ...
        if not local_seed.exists():
            raise RuntimeError(f"Missing local seed for World Curling Championships: {local_seed}")

        frame = pd.read_csv(local_seed)
        if "year" not in frame.columns:
            raise RuntimeError(f"Unsupported World Curling Championships seed format for {local_seed.name}.")
        ...
        if filtered.empty:
            raise RuntimeError(f"No World Curling Championships rows for year <= {season_year} and year > 2000.")
        ...
        self._write_json(
            out_dir / "fetch_meta.json",
            {
                ...
                "excluded_scope": "Wheelchair curling tables are not included in this connector.",
            },
        )
        return [out_file]
    ...
    def parse(self, raw_paths: list[Path], season_year: int) -> dict[str, pd.DataFrame]:
        ...
        if not required_cols.issubset(set(frame.columns)):
            raise RuntimeError(f"Unsupported World Curling Championships seed columns: {list(frame.columns)}")
        ...
        if (frame["year"] <= 2000).any():
            ...
            raise RuntimeError(f"Post-2000 guard violated for World Curling Championships seed: {offenders}")
        ...
        bad_profiles = {key: value for key, value in rank_profiles.items() if value != (1, 2, 3)}
        if bad_profiles:
            sample = dict(list(bad_profiles.items())[:20])
            raise RuntimeError(f"Unexpected World Curling Championships rank profiles: {sample}")

        timestamp = utc_now_iso()
        sports_df = pd.DataFrame(
            [{"sport_id": SPORT_ID, "sport_name": "Curling", "sport_slug": SPORT_ID, "created_at_utc": timestamp}]
        )
        ...
        competitions_df["sport_id"] = SPORT_ID
            ...
            results_rows.append(
                {
                    ...
                    "medal": str(row.medal),
                    "score_raw": (
                        f"event={row.event_name};source_country={country_name};"
    ...
    def _assert_result_profiles(results_df: pd.DataFrame) -> None:
        ...
        if bad_profiles:
            sample = dict(list(bad_profiles.items())[:20])
            raise RuntimeError(f"Unexpected World Curling Championships result profiles: {sample}")
