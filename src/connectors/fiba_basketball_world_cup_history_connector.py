...
COMPETITIONS: dict[str, dict[str, str]] = {
    "men": {
        "seed_file": "fiba_world_cup_men_top4_seed.csv",
        "competition_id": "fiba_basketball_world_cup_men",
        "competition_name": "FIBA Basketball World Cup (Men)",
        "discipline_name": "FIBA Basketball World Cup Men Final Ranking",
        "gender": "men",
    },
    "women": {
        "seed_file": "fiba_world_cup_women_top4_seed.csv",
        "competition_id": "fiba_basketball_world_cup_women",
        "competition_name": "FIBA Women's Basketball World Cup",
        "discipline_name": "FIBA Basketball World Cup Women Final Ranking",
        "gender": "women",
    },
}


class FibaBasketballWorldCupHistoryConnector(Connector):
    id = "fiba_basketball_world_cup_history"
    name = "FIBA Basketball World Cup Historical Results (Men/Women)"
    source_type = "csv"
    license_notes = (
        "Historical top4 seeds curated from public FIBA / Wikipedia references. "
    ...
    def source_row(self) -> dict[str, str]:
        return {
            ...
            "license_notes": (
                ...
                "data/raw/basketball/fiba_world_cup_men_top4_seed.csv and "
                "data/raw/basketball/fiba_world_cup_women_top4_seed.csv "
                "(curated from FIBA / Wikipedia public information)."
            ),
            "base_url": self.base_url,
        }

    def _local_seed_path(self, seed_file: str) -> Path:
        return Path(__file__).resolve().parents[2] / "data" / "raw" / "basketball" / seed_file
    ...
    def fetch(self, season_year: int, out_dir: Path) -> list[Path]:
        ...
        for meta in COMPETITIONS.values():
            ...
            if not local_seed.exists():
                raise RuntimeError(f"Missing local seed for FIBA basketball world cup history: {local_seed}")
    ...
    def parse(self, raw_paths: list[Path], season_year: int) -> dict[str, pd.DataFrame]:
        ...
        for gender, meta in COMPETITIONS.items():
            ...
            if not required_cols.issubset(set(annual_df.columns)):
                raise RuntimeError(
                    f"Unsupported FIBA basketball world cup seed format for {seed_file}: {list(annual_df.columns)}"
                )
        ...
        if not parsed_by_gender:
            raise RuntimeError("FIBA basketball world cup parsing returned no dataset.")
            ...
            disciplines_rows.append(
                {
                    ...
                    "sport_id": sport_id,
                    "confidence": 1.0,
                    "mapping_source": "connector_fiba_basketball_world_cup_history",
                    "created_at_utc": timestamp,
                }
            )
                    ...
                    results_rows.append(
                        {
                            ...
                            "rank": rank,
                            "medal": medal,
                            "score_raw": f"fiba_basketball_world_cup_final_rank={rank}",
    ...
    def upsert(self, db: SQLiteDB, payload: dict[str, pd.DataFrame]) -> None:
        with db.connect() as conn:
            ...
            conn.execute("DELETE FROM disciplines WHERE mapping_source = 'connector_fiba_basketball_world_cup_history'")
            conn.commit()
