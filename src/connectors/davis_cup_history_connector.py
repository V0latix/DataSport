...
SEED_FILE = "davis_cup_finalists_seed.csv"
...
class DavisCupHistoryConnector(Connector):
    id = "davis_cup_history"
    name = "Davis Cup Historical Finalists"
    source_type = "csv"
    license_notes = (
        "Historical finalists seed curated from public Wikipedia List of Davis Cup champions table. "
        "Verify downstream redistribution requirements."
    )
    base_url = "https://en.wikipedia.org/wiki/List_of_Davis_Cup_champions"
    ...
    def source_row(self) -> dict[str, str]:
        return {
            ...
            "license_notes": (
                "Local seed data/raw/tennis/davis_cup_finalists_seed.csv built from Wikipedia "
                "List of Davis Cup champions. Strict post-2000 scope; Davis Cup has no third-place match, "
                "so only finalists are stored with rank profile 1,2. RTF is preserved as source-domain code."
            ),
    ...
    def fetch(self, season_year: int, out_dir: Path) -> list[Path]:
        ...
        if not local_seed.exists():
            raise RuntimeError(f"Missing local seed for Davis Cup: {local_seed}")

        frame = pd.read_csv(local_seed)
        if "year" not in frame.columns:
            raise RuntimeError(f"Unsupported Davis Cup seed format for {local_seed.name}: missing `year`.")
        ...
        if filtered.empty:
            raise RuntimeError(f"No Davis Cup rows for year <= {season_year} and year > 2000.")
    ...
    def parse(self, raw_paths: list[Path], season_year: int) -> dict[str, pd.DataFrame]:
        ...
        if not required_cols.issubset(set(frame.columns)):
            raise RuntimeError(f"Unsupported Davis Cup seed columns: {list(frame.columns)}")
        ...
        if (frame["year"] <= 2000).any():
            ...
            raise RuntimeError(f"Post-2000 guard violated for Davis Cup seed: {offenders}")
            ...
            results_rows.append(
                {
                    ...
                    "medal": str(row.medal),
                    "score_raw": (
                        f"davis_cup_final_rank={rank};score={row.score};source_year={row.source_year};"
    ...
    def _assert_result_profiles(results_df: pd.DataFrame) -> None:
        ...
        if bad_profiles:
            sample = dict(list(bad_profiles.items())[:20])
            raise RuntimeError(f"Unexpected Davis Cup result profiles: {sample}")
