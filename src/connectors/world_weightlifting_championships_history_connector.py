...
SEED_FILE = "world_weightlifting_championships_top3_seed.csv"
...
class WorldWeightliftingChampionshipsHistoryConnector(Connector):
    id = "world_weightlifting_championships_history"
    name = "World Weightlifting Championships Historical Podiums"
    source_type = "csv"
    license_notes = (
        "Historical podium seed curated from public Wikipedia World Weightlifting Championships medalist lists. "
        "Verify downstream redistribution requirements."
    )
    base_url = "https://en.wikipedia.org/wiki/World_Weightlifting_Championships"
    ...
    def source_row(self) -> dict[str, str]:
        return {
            ...
            "license_notes": (
                "Local seed data/raw/weightlifting/world_weightlifting_championships_top3_seed.csv built from "
                "Wikipedia men/women World Weightlifting Championships medalist list tables. Strict post-2000 "
                "scope; stores total podiums only with rank profile 1,2,3."
            ),
            "base_url": self.base_url,
        }

    def _local_seed_path(self) -> Path:
        return Path(__file__).resolve().parents[2] / "data" / "raw" / "weightlifting" / SEED_FILE
    ...
    def fetch(self, season_year: int, out_dir: Path) -> list[Path]:
        ...
        if not local_seed.exists():
            raise RuntimeError(f"Missing local seed for World Weightlifting Championships: {local_seed}")

        frame = pd.read_csv(local_seed)
        if "year" not in frame.columns:
            raise RuntimeError(f"Unsupported World Weightlifting seed format for {local_seed.name}: missing `year`.")
        ...
        if filtered.empty:
            raise RuntimeError(f"No World Weightlifting rows available for year <= {season_year} and year > 2000.")
    ...
    def parse(self, raw_paths: list[Path], season_year: int) -> dict[str, pd.DataFrame]:
        ...
        if not required_cols.issubset(set(frame.columns)):
            raise RuntimeError(f"Unsupported World Weightlifting seed format for {seed_path.name}: {list(frame.columns)}")
        ...
        if (frame["year"] <= 2000).any():
            ...
            raise RuntimeError(f"Post-2000 guard violated for World Weightlifting seed: {offenders}")
        ...
        if bad_profiles:
            ...
            raise RuntimeError(f"Unexpected World Weightlifting rank profiles: {sample}")

        timestamp = utc_now_iso()
        sports_df = pd.DataFrame(
            [{"sport_id": SPORT_ID, "sport_name": "Weightlifting", "sport_slug": SPORT_ID, "created_at_utc": timestamp}]
        )
        ...
        for row in frame.itertuples(index=False):
            ...
            participant_id = f"world_weightlifting_athlete_{self._clean_participant_name_for_id(participant_name)}_{country_code}"
            ...
            results_rows.append(
                {
                    ...
                    "score_raw": (
                        ...
                        f"weight_class={row.weight_class};games={row.games_label};"
    ...
    def _assert_result_profiles(results_df: pd.DataFrame) -> None:
        ...
        if bad_profiles:
            sample = dict(list(bad_profiles.items())[:30])
            raise RuntimeError(f"Unexpected World Weightlifting result profiles after participant build: {sample}")
