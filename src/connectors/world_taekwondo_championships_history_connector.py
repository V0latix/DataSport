...
SEED_FILE = "world_taekwondo_championships_top4_seed.csv"
...
class WorldTaekwondoChampionshipsHistoryConnector(Connector):
    id = "world_taekwondo_championships_history"
    name = "World Taekwondo Championships Historical Podiums (Top 4 by weight category)"
    source_type = "csv"
    license_notes = (
        "Historical podium seed curated from official World Taekwondo medalists pages. "
        "Verify downstream redistribution requirements."
    )
    base_url = "https://web.worldtaekwondo.martial.services/competitions?type=wc"
    ...
    def source_row(self) -> dict[str, str]:
        return {
            ...
            "license_notes": (
                "Local seed data/raw/taekwondo/world_taekwondo_championships_top4_seed.csv built from "
                "official World Taekwondo medalist pages (strict post-2000 scope). "
                "Expected rank profiles per event: 1,2,3,3 or official single-bronze profile 1,2,3; "
                "non-standard source anomalies are excluded by the seed builder."
            ),
            "base_url": self.base_url,
        }

    def _local_seed_path(self) -> Path:
        return Path(__file__).resolve().parents[2] / "data" / "raw" / "taekwondo" / SEED_FILE
    ...
    @staticmethod
    def _event_id(year: int, gender: str, weight_class: str) -> str:
        return f"world_taekwondo_championships_{year}_{slugify(gender)}_{slugify(weight_class)}"

    def fetch(self, season_year: int, out_dir: Path) -> list[Path]:
        local_seed = self._local_seed_path()
        if not local_seed.exists():
            raise RuntimeError(f"Missing local seed for World Taekwondo Championships history: {local_seed}")

        frame = pd.read_csv(local_seed)
        if "year" not in frame.columns:
            raise RuntimeError(f"Unsupported World Taekwondo seed format for {local_seed.name}: missing `year`.")
        ...
        if filtered.empty:
            raise RuntimeError(f"No World Taekwondo rows available for year <= {season_year} and year > 2000.")
    ...
    def parse(self, raw_paths: list[Path], season_year: int) -> dict[str, pd.DataFrame]:
        ...
        if not required_cols.issubset(set(frame.columns)):
            raise RuntimeError(f"Unsupported World Taekwondo seed format for {seed_path.name}: {list(frame.columns)}")
        ...
        frame["weight_class"] = frame["weight_class"].fillna("").astype(str).str.strip().str.lower()
        ...
        frame = frame.loc[
            ...
            & (frame["competition_id"] == "world_taekwondo_championships")
        ...
        if (frame["year"] <= 2000).any():
            ...
            raise RuntimeError(f"Post-2000 guard violated for World Taekwondo seed: {offenders}")
        ...
        if bad_profiles:
            ...
            raise RuntimeError(f"Unexpected World Taekwondo rank profiles: {sample}")
        ...
        disciplines_df = pd.DataFrame(
            [
                {
                    "discipline_id": "taekwondo",
                    "discipline_name": "Taekwondo",
                    "discipline_slug": "taekwondo",
                    "sport_id": "taekwondo",
                    "confidence": 1.0,
                    "mapping_source": "connector_world_taekwondo_championships_history",
                    "created_at_utc": timestamp,
                }
            ]
        )
        ...
        for year, gender, weight_class, event_date in (
            frame[["year", "gender", "weight_class", "event_date"]]
            .drop_duplicates()
            ...
            events_rows.append(
                {
                    "event_id": self._event_id(int(year), str(gender), str(weight_class)),
                    "competition_id": "world_taekwondo_championships",
                    "discipline_id": "taekwondo",
                    "gender": str(gender),
                    "event_class": "podium_top4_by_weight_category",
        ...
        for row in frame.itertuples(index=False):
            ...
            rank = int(row.rank)
            event_id = self._event_id(int(row.year), str(row.gender), str(row.weight_class))
            score_raw = (
                f"discipline=taekwondo;weight_class={row.weight_class};event={row.event_name};"
    ...
    def _assert_result_profiles(results_df: pd.DataFrame) -> None:
        ...
        if bad_profiles:
            sample = dict(list(bad_profiles.items())[:30])
            raise RuntimeError(f"Unexpected World Taekwondo result profiles after participant remap: {sample}")
