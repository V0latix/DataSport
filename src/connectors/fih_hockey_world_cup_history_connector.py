...
SEED_FILE = "fih_hockey_world_cup_top4_seed.csv"
COMPETITION_IDS = {
...
class FihHockeyWorldCupHistoryConnector(Connector):
    id = "fih_hockey_world_cup_history"
    name = "FIH Hockey World Cup Historical Results (Men/Women Top 4)"
    source_type = "csv"
    license_notes = (
        "Historical top4 seed curated from public Wikipedia FIH Hockey World Cup pages. "
        "Verify downstream redistribution requirements."
    )
    base_url = (
        "https://en.wikipedia.org/wiki/Men%27s_FIH_Hockey_World_Cup | "
        "https://en.wikipedia.org/wiki/Women%27s_FIH_Hockey_World_Cup"
    )
    ...
    def source_row(self) -> dict[str, str]:
        return {
            ...
            "license_notes": (
                "Local seed data/raw/hockey/fih_hockey_world_cup_top4_seed.csv built from "
                "Wikipedia men's and women's FIH Hockey World Cup final tables (top4, strict post-2000 scope)."
            ),
    ...
    def fetch(self, season_year: int, out_dir: Path) -> list[Path]:
        ...
        if not local_seed.exists():
            raise RuntimeError(f"Missing local seed for FIH Hockey World Cup history: {local_seed}")

        frame = pd.read_csv(local_seed)
        if "year" not in frame.columns:
            raise RuntimeError(f"Unsupported FIH seed format for {local_seed.name}: missing `year`.")
        ...
        if filtered.empty:
            raise RuntimeError(f"No FIH rows available for year <= {season_year} and year > 2000.")
    ...
    def parse(self, raw_paths: list[Path], season_year: int) -> dict[str, pd.DataFrame]:
        ...
        if not required_cols.issubset(set(frame.columns)):
            raise RuntimeError(f"Unsupported FIH seed format for {seed_path.name}: {list(frame.columns)}")
        ...
        if (frame["year"] <= 2000).any():
            ...
            raise RuntimeError(f"Post-2000 guard violated for FIH seed: {offenders}")
        ...
        if bad_profiles:
            ...
            raise RuntimeError(f"Unexpected FIH rank profiles (expected 1,2,3,4): {sample}")
            ...
            [
                {
                    ...
                    "sport_id": "hockey",
                    "confidence": 1.0,
                    "mapping_source": "connector_fih_hockey_world_cup_history",
                    "created_at_utc": timestamp,
                }
            ]
        )
        ...
        for row in frame.itertuples(index=False):
            ...
            score_raw = f"fih_hockey_world_cup_final_rank={rank};source_url={row.source_url}"
            results_rows.append(
                {
                    "event_id": event_id,
                    "participant_id": participant_id,
    ...
    def upsert(self, db: SQLiteDB, payload: dict[str, pd.DataFrame]) -> None:
        ...
        with db.connect() as conn:
            ...
            conn.execute(
                ...
                WHERE mapping_source = 'connector_fih_hockey_world_cup_history'
