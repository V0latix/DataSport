...
SEED_FILE = "world_lacrosse_womens_world_cup_top4_seed.csv"
COMPETITION_ID = "world_lacrosse_womens_world_cup"
SPORT_ID = "lacrosse"
DISCIPLINE_ID = "field-lacrosse"
...
class WorldLacrosseWomensWorldCupHistoryConnector(Connector):
    id = "world_lacrosse_womens_world_cup_history"
    name = "Women's Lacrosse World Cup Historical Top 4"
    source_type = "csv"
    license_notes = (
        "Historical top4 seed curated from public Wikipedia World Lacrosse Women's Championship tables. "
        "Verify downstream redistribution requirements."
    )
    base_url = "https://en.wikipedia.org/wiki/World_Lacrosse_Women%27s_Championship"
    ...
    def source_row(self) -> dict[str, str]:
        return {
            ...
            "license_notes": (
                "Local seed data/raw/lacrosse/world_lacrosse_womens_world_cup_top4_seed.csv built from "
                "Wikipedia World Lacrosse Women's Championship past results table. Strict post-2000 scope; "
                "stores women's national-team top4 final standings with rank profile 1,2,3,4."
            ),
            "base_url": self.base_url,
        }

    def _local_seed_path(self) -> Path:
        return Path(__file__).resolve().parents[2] / "data" / "raw" / "lacrosse" / SEED_FILE
    ...
    def fetch(self, season_year: int, out_dir: Path) -> list[Path]:
        ...
        if not local_seed.exists():
            raise RuntimeError(f"Missing local seed for Women's Lacrosse World Cup: {local_seed}")

        frame = pd.read_csv(local_seed)
        if "year" not in frame.columns:
            raise RuntimeError(f"Unsupported Women's Lacrosse seed format for {local_seed.name}: missing `year`.")
        ...
        if filtered.empty:
            raise RuntimeError(f"No Women's Lacrosse rows for year <= {season_year} and year > 2000.")
        ...
        self._write_json(
            out_dir / "fetch_meta.json",
            {
                ...
                "non_championship_or_unavailable_years_up_to_requested": missing_years,
            },
        )
        return [out_file]
    ...
    def parse(self, raw_paths: list[Path], season_year: int) -> dict[str, pd.DataFrame]:
        ...
        if not required_cols.issubset(set(frame.columns)):
            raise RuntimeError(f"Unsupported Women's Lacrosse seed columns: {list(frame.columns)}")
        ...
        if (frame["year"] <= 2000).any():
            ...
            raise RuntimeError(f"Post-2000 guard violated for Women's Lacrosse seed: {offenders}")
        ...
        if bad_profiles:
            ...
            raise RuntimeError(f"Unexpected Women's Lacrosse rank profiles: {sample}")

        timestamp = utc_now_iso()
        sports_df = pd.DataFrame(
            [{"sport_id": SPORT_ID, "sport_name": "Lacrosse", "sport_slug": SPORT_ID, "created_at_utc": timestamp}]
        )
            ...
            [
                {
                    ...
                    "sport_id": SPORT_ID,
                    "name": "Women's Lacrosse World Cup",
                    "season_year": None,
            ...
            results_rows.append(
                {
                    ...
                    "rank": rank,
                    "medal": str(row.medal),
                    "score_raw": f"world_lacrosse_womens_world_cup_final_rank={rank};source_url={row.source_url}",
    ...
    def _assert_result_profiles(results_df: pd.DataFrame) -> None:
        ...
        if bad_profiles:
            sample = dict(list(bad_profiles.items())[:20])
            raise RuntimeError(f"Unexpected Women's Lacrosse result profiles: {sample}")
