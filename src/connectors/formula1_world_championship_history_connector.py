...
SEED_FILE = "formula1_world_standings_top10_seed.csv"
COMPETITION_IDS = {
...
class Formula1WorldChampionshipHistoryConnector(Connector):
    id = "formula1_world_championship_history"
    name = "Formula 1 World Championship Final Standings (Top 10 Drivers + Constructors)"
    source_type = "csv"
    license_notes = (
        "Historical yearly final standings from the public Jolpica/Ergast API, materialized as a local seed. "
    ...
    def source_row(self) -> dict[str, str]:
        return {
            ...
            "license_notes": (
                "Local seed data/raw/formula1/formula1_world_standings_top10_seed.csv "
                "built from Jolpica/Ergast annual driverStandings + constructorStandings endpoints. "
    ...
    def _local_seed_path(self) -> Path:
        return Path(__file__).resolve().parents[2] / "data" / "raw" / "formula1" / SEED_FILE
    ...
    def _participant_id(participant_type: str, participant_ref: str, participant_name: str) -> str:
        prefix = "driver" if participant_type == "athlete" else "constructor"
    ...
    def fetch(self, season_year: int, out_dir: Path) -> list[Path]:
        ...
        if not local_seed.exists():
            raise RuntimeError(f"Missing local seed for Formula 1 world championship history: {local_seed}")

        frame = pd.read_csv(local_seed)
        if "year" not in frame.columns:
            raise RuntimeError(f"Unsupported F1 seed format for {local_seed.name}: missing `year`.")
        ...
        if filtered.empty:
            raise RuntimeError(
                f"No Formula 1 rows available for completed seasons <= {max_completed_season} and year > 2000."
            )
    ...
    def parse(self, raw_paths: list[Path], season_year: int) -> dict[str, pd.DataFrame]:
        ...
        if not required_cols.issubset(set(frame.columns)):
            raise RuntimeError(f"Unsupported Formula 1 seed format for {seed_path.name}: {list(frame.columns)}")
        ...
        frame = frame.loc[
            ...
            & (frame["discipline_key"] == "formula-one")
            & (frame["gender"] == "mixed")
        ...
        if (frame["year"] <= 2000).any():
            ...
            raise RuntimeError(f"Post-2000 guard violated for Formula 1 seed: {offenders}")
        ...
        if bad_profiles:
            ...
            raise RuntimeError(f"Unexpected Formula 1 rank profiles (expected 1..10): {sample}")
            ...
            [
                {
                    ...
                    "discipline_slug": "formula-one",
                    "sport_id": "motorsport",
                    "confidence": 1.0,
                    "mapping_source": "connector_formula1_world_championship_history",
                    "created_at_utc": timestamp,
                }
            ]
        )
        ...
        for row in frame.itertuples(index=False):
            ...
            participant_id = self._participant_id(str(row.participant_type), str(row.participant_ref), str(row.participant_name))
            ...
            if country_code:
                if country_code not in countries_rows:
                    ...
                    countries_rows[country_code] = {
                        ...
                        "iso3": country_code,
                        "name_en": getattr(country_obj, "name", country_name) if country_obj else (country_name or country_code),
                        "name_fr": None,
                    }
            ...
            score_raw = (
                ...
                f"nationality={row.nationality};round={row.round};source_url={row.source_url}"
            )
            results_rows.append(
                {
                    "event_id": event_id,
    ...
    def upsert(self, db: SQLiteDB, payload: dict[str, pd.DataFrame]) -> None:
        with db.connect() as conn:
            ...
            conn.execute(
                ...
                WHERE mapping_source = 'connector_formula1_world_championship_history'
